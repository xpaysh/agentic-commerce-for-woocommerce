...
9. **agent-commerce.xpay.sh/v1/merchants/{slug}/orders** — Agent-attributed order reporting. On a connected store, when an order reaches a paid status the plugin POSTs a **non-PII** summary so your dashboard can show which AI channel drove the sale: the WooCommerce order id, placed-at time, status, order total and discount, currency, line count, the ordered SKUs, and the attribution source. Depending on the order it may also carry WooCommerce core's own order-source context (channel type, device, and the *path* — host, query and fragment stripped — of the page the shopper first landed on) and, for an order completed through WooCommerce's Agentic Checkout, that checkout's session and provider id. **Never sent:** customer name, email, address, phone, IP, or payment details. Fires only for connected stores and can be turned off entirely with the `xpay_wc_order_events_enabled` option. This is the same reporting described in the Changelog for 0.4.3–0.5.2.
